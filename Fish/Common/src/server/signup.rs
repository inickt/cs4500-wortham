...
pub fn signup_clients(port: usize, client_timeout: Duration) -> Option<Vec<Box<dyn Client>>> {
...
fn await_clients(listener: &TcpListener, clients: &mut Vec<Box<dyn Client>>, client_timeout: Duration) {
    let now = Instant::now();
